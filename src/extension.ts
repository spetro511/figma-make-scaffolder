...
interface ScaffoldConfig {
    ...
    generateComponentPreview?: boolean;
}
...
async function loadConfig(context: vscode.ExtensionContext): Promise<ScaffoldConfig> {
    const config = context.globalState.get<ScaffoldConfig>('figmaMakeConfig', {});
    return config;
}

async function saveConfig(context: vscode.ExtensionContext, config: ScaffoldConfig): Promise<void> {
...
async function generateComponentPreview(projectDir: string): Promise<void> {
    try {
        const srcDir = path.join(projectDir, 'src');
        const components: Array<{ name: string; path: string; exports: string[] }> = [];
        
        // Find all component files
        const findComponents = async (dir: string, basePath: string = '') => {
            const entries = await fs.promises.readdir(dir, { withFileTypes: true });
...
<body>
    ...
    <div class="component-grid">
        ...
        vscode.window.showInformationMessage(
            `Component preview generated with ${components.length} components`,
            'Open Preview'
        ).then(selection => {
